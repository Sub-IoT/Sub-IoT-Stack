...
continuous_tx(uint8_t* data){
    ...
    if(data[0] & 0x10) {
        ...
        //TODO add when rx done
    } else log_print_string("send random");
    ...
    /* Configure */
    if(current_modulation == MODULATION_CW){
...
engin_file_change_callback(uint8_t file_id){
    ...
    if(data[0] & 0x01){ 
        log_print_string("tx");
        continuous_tx(data);
    } else log_print_string("rx");

    log_print_string("file changed\n");
}
