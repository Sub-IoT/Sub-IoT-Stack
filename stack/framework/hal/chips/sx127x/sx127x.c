...
int16_t hw_radio_get_rssi() {
    hw_radio_set_opmode(HW_STATE_RX); //400 µs in RX before return
    hw_gpio_disable_interrupt(SX127x_DIO1_PIN);
    hw_busy_wait(rx_bw_startup_time[rx_bw_number]); //900 µs in RX before return
    return (- read_reg(REG_RSSIVALUE) >> 1); //400 µs in RX before in standby
}
