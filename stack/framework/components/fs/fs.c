...
#define FS_BLOCKDEVICES_COUNT   (FS_BLOCKDEVICE_TYPE_VOLATILE + 1) // TODO + extra
...
static int _fs_init_permanent_systemfiles(uint8_t* permanent_systemfiles);
...
void fs_init(uint8_t* provisioning)
{
    ...
    bd[FS_BLOCKDEVICE_TYPE_PERMANENT] = PLATFORM_PERMANENT_BD;
    bd[FS_BLOCKDEVICE_TYPE_VOLATILE] = PLATFORM_VOLATILE_BD;
#endif
...
int _fs_init_permanent_systemfiles(uint8_t* provisioning)
{
    ...
    {
        _fs_create_file(file_id, permanent_systemfiles->files_data + permanent_systemfiles->files_offset[file_id],
                        permanent_systemfiles->files_length[file_id]);
    }
#else
    DPRINT("metadata addr: %x", provisioning);
    DPRINT("metadata nfiles: %x", provisioning + FS_NUMBER_OF_FILES_ADDRESS);
    // initialise system file caching
    uint8_t number_of_files;
    blockdevice_read(bd[FS_BLOCKDEVICE_TYPE_PERMANENT], (uint8_t*)&number_of_files, FS_NUMBER_OF_FILES_ADDRESS, FS_NUMBER_OF_FILES_SIZE);
    assert(number_of_files < FRAMEWORK_FS_FILE_COUNT);
    for(int file_id = 0; file_id < FRAMEWORK_FS_FILE_COUNT; file_id++)
    {
        blockdevice_read(bd[FS_BLOCKDEVICE_TYPE_PERMANENT], (uint8_t*)&files[file_id],
        ...
        DPRINT("Loading file %i addr %i len %i\n", file_id, files[file_id].addr, files[file_id].length);
        if(_is_file_defined(file_id))
        {
            switch(files[file_id].blockdevice_index)
            {
                ...
                {
                    // TODO this can be removed in the future. For now we assume volatile files to be generated in FS as well
...
static int _fs_verify_magic(uint8_t* expected_magic_number)
{
    ...
    blockdevice_read(bd[FS_BLOCKDEVICE_TYPE_PERMANENT], magic_number, 0, FS_MAGIC_NUMBER_SIZE);
    if(memcmp(expected_magic_number, magic_number, FS_MAGIC_NUMBER_SIZE) != 0)
        return -EFAULT;
#endif
...
int _fs_create_file(uint8_t file_id, fs_storage_class_t storage_class, const uint8_t* initial_data, uint32_t length)
{
    ...
    if (bd_type == FS_BLOCKDEVICE_TYPE_PERMANENT)
    {
        ...
        blockdevice_program(bd[FS_BLOCKDEVICE_TYPE_PERMANENT], (uint8_t*)&file_header_big_endian, _get_file_header_address(file_id), FS_FILE_HEADER_SIZE);
#else
        blockdevice_program(bd[FS_BLOCKDEVICE_TYPE_PERMANENT], (uint8_t*)&files[file_id], _get_file_header_address(file_id), FS_FILE_HEADER_SIZE);
#endif
