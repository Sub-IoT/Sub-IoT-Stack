...
MACRO (GEN_SETTINGS_INI section_name)
    cmake_parse_arguments(_GSH "" "" "STRING;BOOL;ID;NUMBER" ${ARGN} )

    IF(NOT EXISTS "${CONFIGURATION_FILE}")
    FILE(WRITE  "${CONFIGURATION_FILE}" "#Warning: This file is automatically generated by CMake. Do NOT EDIT this file directly !!\r\n")
    ENDIF()

    STRING(TOUPPER "${section_name}" section)
    STRING(REGEX REPLACE "[^a-zA-Z0-9_]" "_" section "${section}")

    FILE(APPEND "${CONFIGURATION_FILE}" "[${section}]\r\n")

    FOREACH(var ${_GSH_STRING})
	FILE(APPEND "${CONFIGURATION_FILE}" "${var} = \"${${var}}\"\r\n")	    
    ENDFOREACH()
    FOREACH(var ${_GSH_ID} ${_GSH_NUMBER})
	FILE(APPEND "${CONFIGURATION_FILE}" "${var} = ${${var}}\r\n")	    
    ENDFOREACH()
    FOREACH(var ${_GSH_BOOL})
	IF(${var})
	    FILE(APPEND "${CONFIGURATION_FILE}" "${var} = True\r\n")	    
	ENDIF()
    ENDFOREACH()
