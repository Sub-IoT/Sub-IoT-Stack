...
#define D7A_FILE_FIRMWARE_VERSION_SIZE                                                                                 \
    (2 + 2 + D7A_FILE_FIRMWARE_VERSION_APP_NAME_SIZE + D7A_FILE_FIRMWARE_VERSION_GIT_SHA1_SIZE)
...
#define D7A_FILE_NWL_SECURITY_STATE_REG_SIZE                                                                           \
    2 + (FRAMEWORK_FS_TRUSTED_NODE_TABLE_SIZE) * (D7A_FILE_NWL_SECURITY_SIZE + D7A_FILE_UID_SIZE)

#define D7AP_FS_SYSTEMFILES_COUNT                                                                                      \
    0x2F // reserved up until 0x3F but used only until 0x2F so use this for limiting memory usage
 ...
 * \param length    a pointer to the length of the data which will be written to the file
 * **/
typedef bool (*d7ap_fs_modifying_file_callback_t)(
    uint8_t file_id, uint32_t* offset, const uint8_t* buffer, uint32_t* length);
...
int d7ap_fs_write_file_with_callback(
    uint8_t file_id, uint32_t offset, const uint8_t* buffer, uint32_t length, authentication_t auth, bool trigger_cb);
