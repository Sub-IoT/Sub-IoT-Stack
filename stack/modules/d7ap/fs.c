...
#define FILE_DATA_SIZE 20 // TODO define from cmake (per platform?)
...
void fs_init()
{
    ...
    // TODO file_headers[D7A_FILE_UID_FILE_ID]
    uint64_t id = hw_get_unique_id();
    memcpy(data_ptr, &id, D7A_FILE_UID_LENGTH);
    data_ptr += D7A_FILE_UID_LENGTH;
    ...
    // TODO file_headers[D7A_FILE_ACCESS_PROFILE_ID]
...
{
    assert(access_class_index < 16);
    //assert(file_headers[D7A_FILE_ACCESS_PROFILE_ID + access_class_index] != 0) // TODO first write the header
    uint8_t* data_ptr = data + file_offsets[D7A_FILE_ACCESS_PROFILE_ID + access_class_index];
