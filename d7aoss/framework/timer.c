...
bool timer_insert_value_in_queue(timer_event* event)
{
	...
	log_print_stack_string(LOG_FWK, "timer_insert S - Timer current value: %u", current_timer);
	#endif
	...
	{
		timer_event *temp_event = (timer_event*) queue_read_value(&event_queue, position, sizeof(timer_event));
		log_print_stack_string(LOG_FWK, "Queue: %u", temp_event->next_event);
	}
	current_timer = hal_timer_getvalue();
	log_print_stack_string(LOG_FWK, "timer_insert E - Timer current value: %u", current_timer);
	#endif
	/*
...
bool timer_add_event(timer_event* event)
{
	#ifdef LOG_FWK_ENABLED
	uint16_t current_timer = hal_timer_getvalue();
	log_print_stack_string(LOG_FWK, "timer_add_event: %u", event->next_event);
	log_print_stack_string(LOG_FWK, "timer_add_event timer S : %u", current_timer);
	#endif
    ...
    if (timer_insert_value_in_queue(&new_event))
    {
        ...
        if (diff < 1000)
		{
			#ifdef LOG_FWK_ENABLED
        	current_timer = hal_timer_getvalue();
        	log_print_stack_string(LOG_FWK, "timer_add_event M timer overrun : %u", current_timer);
        	log_print_stack_string(LOG_FWK, "timer_add_event M: %u", new_event.next_event);
			#endif
			timer_completed();
		}
    } else {
	...
	#ifdef LOG_FWK_ENABLED
	current_timer = hal_timer_getvalue();
	log_print_stack_string(LOG_FWK, "timer_add_event timer E : %u", current_timer);
	#endif
...
void timer_completed()
{
	#ifdef LOG_FWK_ENABLED
	uint16_t current_timer = hal_timer_getvalue();
	log_print_stack_string(LOG_FWK, "timer_completed: %u", current_timer);
	#endif
